...
def _inline_render_disabled() -> bool:
    return os.environ.get("LM_PDF_INLINE_RENDER") == "0"


def _files_url_base() -> str | None:
    """Return base URL for the /files HTTP route, or None when no HTTP server
    is active. Clients (e.g. llama.cpp WebUI) that drop ImageContent can render
    images via this URL instead of getting a base64 blob baked into the prompt.

    LM_PDF_FILES_URL_BASE overrides; otherwise built from MCP_HOST + MCP_PORT
    when MCP_TRANSPORT=http."""
    if os.environ.get("MCP_TRANSPORT") != "http":
        return None
    override = os.environ.get("LM_PDF_FILES_URL_BASE")
    if override:
        return override.rstrip("/")
    host = os.environ.get("MCP_HOST", "127.0.0.1")
    port = os.environ.get("MCP_PORT", "8092")
...
def pdf_render_page(path: str, page: int, dpi: int = _DPI_DEFAULT) -> list:
    ...
    Set LM_PDF_INLINE_RENDER=0 to suppress the ImageContent block; only the
    metadata + cache path is returned. Useful when accumulated inline images
    blow past the model context window. Under HTTP transport, the text also
    contains a markdown image link to the /files HTTP route so browser-based
    clients (llama.cpp WebUI) display the page without the base64 ever
    entering the prompt context. To surface that image in your assistant
    reply (not just inside the tool-result block), reproduce the markdown
    image line verbatim in your reply text."""
    target = _open_target(path)
    parsed = _get_parsed(target)
    total = parsed["meta"]["page_count"]
    if not (1 <= page <= total):
    ...
    if not cache_disabled and render_path.exists():
        ...
        if cache_disabled:
            ...
            try:
                ...
                cache_note = f"# disk cache: ./{render_path.relative_to(_root())}"
            except OSError as e:
    ...
    if files_base is not None and not _cache_disabled():
        rel = render_path.relative_to(_cache_dir()).as_posix()
        text_lines.append(
            "# include the next markdown line verbatim in your reply so the user sees the page in chat:"
        )
        text_lines.append(
            f"![page {page}]({files_base}/{quote(rel, safe='/')})"
        )

    inline_disabled = _inline_render_disabled()
    if inline_disabled:
        text_lines.append("# inline image suppressed: LM_PDF_INLINE_RENDER=0")
...
def pdf_extract_region(
    ...
    path), ImageContent (PNG)]. Under HTTP transport, the text also contains
    a markdown image link to the /files HTTP route so browser-based clients
    (llama.cpp WebUI) display the crop without the base64 ever entering the
    prompt context. To surface that image in your assistant reply (not just
    inside the tool-result block), reproduce the markdown image line
    verbatim in your reply text."""
    target = _open_target(path)
    parsed = _get_parsed(target)
    total = parsed["meta"]["page_count"]
    if not (1 <= page <= total):
    ...
    if save_as is not None:
        ...
        save_note = f"# saved to disk: ./{out_path.relative_to(_root())}"
    else:
        ...
        out_path.write_bytes(png_bytes)
        save_note = f"# auto-saved to disk: ./{out_path.relative_to(_root())}"
    ...
    if files_base is not None:
        # Only auto-saved files (in cache dir) are served via HTTP.
        # save_as paths land elsewhere in the sandbox and stay file-only.
        try:
            rel = out_path.relative_to(_cache_dir()).as_posix()
            text_lines.append(
                "# include the next markdown line verbatim in your reply so the user sees the crop in chat:"
            )
            text_lines.append(
                f"![region p{page}]({files_base}/{quote(rel, safe='/')})"
            )
        except ValueError:
            pass
    b64 = base64.b64encode(png_bytes).decode("ascii")
...
_CROSS_ORIGIN_HEADERS = {
    # Browsers (Firefox, Chrome) block <img src=...> from a cross-origin
    # response when CORP is missing — especially when the parent document is
    # in a COEP context like llama.cpp WebUI. Explicitly opt the resource in.
    "Cross-Origin-Resource-Policy": "cross-origin",
    # Defense in depth for canvas reads / fetch() — not needed for plain
    # <img> rendering but cheap and harmless.
    "Access-Control-Allow-Origin": "*",
}


@mcp.custom_route("/files/{rel_path:path}", methods=["GET"])
async def _serve_cache_file(request):
    """Serve PNG (and any other content) from inside .lm-pdf-cache/.

    Browser clients fetch this when rendering the markdown image link emitted
    by pdf_render_page / pdf_extract_region under HTTP transport. Restricted
    to files under the cache dir; any path that resolves outside is rejected.
    Cross-origin headers are set so cross-port browser embedding works.
    """
    from starlette.responses import FileResponse, PlainTextResponse

    rel = request.path_params.get("rel_path", "")
    ...
    try:
        ...
        return PlainTextResponse(
            "Forbidden", status_code=403, headers=_CROSS_ORIGIN_HEADERS,
        )
    if not target.is_file():
        return PlainTextResponse(
            "Not Found", status_code=404, headers=_CROSS_ORIGIN_HEADERS,
        )
    media = "image/png" if target.suffix.lower() == ".png" else "application/octet-stream"
    return FileResponse(target, media_type=media, headers=_CROSS_ORIGIN_HEADERS)
